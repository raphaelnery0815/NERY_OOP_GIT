...
class Logger {
    ...
    public function Log($message) {
        ...
        $logMessage = "$date: ".__CLASS__." $message\n";
...
if ($scheduler->checkAvailability($date, $time, $patient1, "Dr. John Samuel")) {
    echo "APPOINTMENTS \n";
    $logger->Log("Appointment available at $time on $date", "Success");
} else {
    echo "APPOINTMENT IS NOT AVAILABLE AT $time ON $date.\n";
    $logger->Log("Appointment not available at $time on $date", "Error");
}
...
$logger->Log("APPOINTMENT OF: " . $appointment->getPatient()->getName() . " in " . $appointment->getDate() . " - " . $appointment->getTime() . " with ". $appointment->getDoctor() ."\n");
